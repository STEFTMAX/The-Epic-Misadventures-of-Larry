...
import com.steftmax.larrys_epic_misadventures.render.input.MouseScrollListener;
...
public class ChaseCamera implements Camera, MouseScrollListener {
	...
	private float getScaledZoom(final float zoom) {
		final float realXZoom = zoom * Settings.getWidth()
				/ standardResolutionWidth;
		final float realYZoom = zoom * Settings.getHeight()
	...
	public void beginFocus() {
		...
		final int width = Settings.getWidth(), height = Settings.getHeight();
		
		final int halfWindowWidth = width / 2, halfWindowHeight = height / 2;
